...
cd ${TUBBY_PIPELINE}/data/trimmed_fastq/
mkdir -p ${TUBBY_PIPELINE}/results/fastqc_trimmed_reads

for sampleBaseName in "${arrayOfSampleBaseNames[@]}"
do

	echo "  (Note) ignorning the unpaired (aka orphan) reads"
		# doing it like this to avoid the "*un.trim.fastq*" files which are the "unpaired" or "orphan" reads; means its pair was discarded
								# can use, but deciding to ignore
	echo "Running FastQC for 'first pair' of paired-end reads ..."
	fastqc ./${sampleBaseName}_LALL_1.trim.fastq   # makes qc files, html, and zip;  
	echo "Running FastQC for 'second pair' of paired-end reads ..."
	fastqc ./${sampleBaseName}_LALL_2.trim.fastq    # makes qc files, html, and zip;  
        #  don't use multi-thread since not well implemented (slows down val too much)



	echo "Saving FastQC results ..."
	mv -v ./${sampleBaseName}*.zip  ${TUBBY_PIPELINE}/results/fastqc_trimmed_reads
	mv -v ./${sampleBaseName}*.html ${TUBBY_PIPELINE}/results/fastqc_trimmed_reads
done
...
cd ${TUBBY_PIPELINE}/results/fastqc_trimmed_reads

for sampleBaseName in "${arrayOfSampleBaseNames[@]}"
do
    unzip ./${sampleBaseName}_LALL_1.trim_fastqc.zip
    unzip ./${sampleBaseName}_LALL_2.trim_fastqc.zip
    
done

echo "Saving summary to docs (contains all summaries of FastQC per sample) ..."
cat */summary.txt > ${TUBBY_PIPELINE}/docs/fastqc_trimmed_summaries.txt
